<template>
  <div
    ...
    <div class="album__top"
         :style="{borderBottom: `1px solid rgba(${theme}, .3)`}"
    >
      <img class="album__cover" :src="require(`./../../assets/img/home/album_${album.innerTitle}.svg`)" alt="" title="">
      <div class="album__top-right">
        <div class="album__title">{{ album.title }}</div>
        <div class="album__year">Release: {{ album.innerTitle === 'playlist' ? 'No date' : album.date }}</div>
      </div>
    </div>
    <div class="album__songs-wrap">
      <div v-bar>
        <div class="album__songs">
          <div v-if="album.innerTitle === 'playlist' && album.songs.length === 0"
               class="playlist__tooltip"
          >
            <span>Click the 'Edit' to select songs</span>
            <span>Max: {{ playlistLimit }} items</span>
          </div>
          <div class="album__song-wrapper"
          ...
          'move': isPlaylistChanging && album.innerTitle === 'playlist'
            ...
            <svg
              ...
              :class="{disabled: isPlaylistChanging}"
              @click="songDownload(albumIndex, index)"
              width="20" height="16" viewBox="0 0 20 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <title>Download</title>
              <path d="M10 6L10 0" :stroke="isPlaylistChanging ? `rgba(219,219,219, .5)` : `rgb(${theme})`" stroke-width="2"/>
              <path d="M0 15L20 15" :stroke="isPlaylistChanging ? `rgba(219,219,219, .5)` : `rgb(${theme})`" stroke-width="2"/>
              <path d="M13.5116 5.99768L10.007 13.007L6.50232 5.99768L13.5116 5.99768Z" :fill="isPlaylistChanging ? `rgba(219,219,219, .5)` : `rgb(${theme})`"/>
            </svg>
          </div>
        </div>
      </div>
    ...
    <div
      v-if="album.innerTitle === 'playlist'"
      class="playlist__buttons">
      <Button
...
export default {
  ...
  computed: {
    ...
    isDisabled() {return this.album.songs.length === 0 || this.isPlaylistChanging},
    playlistChangeTitle() {return this.isPlaylistChanging ? 'Done' : 'Edit'},
    playlistLimit() {
      let count = 0
  ...
  methods: {
    ...
    songDownload(albumIndex, songIndex) {
      if(!this.isPlaylistChanging) {
        this.downloadSongSrc = this.setDownloadParams(this.albums, albumIndex, songIndex).downloadSrc
        this.downloadSongName = this.setDownloadParams(this.albums, albumIndex, songIndex).downloadName
        this.$emit('downloadSong', {src: this.downloadSongSrc, name: this.downloadSongName})
      }
    },
    downloadAlbum() {
      if(!this.isPlaylistChanging) {
    ...
    moveSong(e) {
      ...
      if(this.moveFromIndex !== this.moveToIndex) {
        ...
        this.album.songs.forEach((item, index) => this.SONG_LEAVE([this.albumIndex, index]))
